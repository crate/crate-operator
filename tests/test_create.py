...
class TestBackup:
    async def does_cronjob_exist(
        self, batchv1beta1: BatchV1beta1Api, namespace: str, name: str
    ) -> bool:
        cjs = await batchv1beta1.list_namespaced_cron_job(namespace=namespace)
    ...
    async def test_not_enabled(self, faker, namespace):
        ...
        ret = await asyncio.gather(
            *create_backups(
                apps, batchv1beta1, namespace.metadata.name, name, {}, {}, None,
            )
        )
        assert ret == []
...
class TestConfigMaps:
    async def do_configmaps_exist(
        self, core: CoreV1Api, namespace: str, expected: Set[str]
    ) -> bool:
        configmaps = await core.list_namespaced_config_map(namespace)
        return expected.issubset({c.metadata.name for c in configmaps.items})

    async def test_create(self, faker, namespace):
        core = CoreV1Api()
        name = faker.domain_word()
        cm_crate, cm_log4j2, cm_sql_exporter = await asyncio.gather(
            *create_configs(core, namespace.metadata.name, name, {}, 74, 80)
        )
        await assert_wait_for(
            True,
            self.do_configmaps_exist,
            core,
            namespace.metadata.name,
            {f"crate-{name}", f"crate-log4j2-{name}", f"crate-sql-exporter-{name}"},
        )
...
class TestStatefulSetContainers:
    def test(self, faker, random_string):
        ...
        assert len(c_sql_exporter.volume_mounts) == 2
...
class TestStatefulSetCrateCommand:
    def test_entrypoint_first_item(self, random_string):
        ...
        assert "/docker-entrypoint.sh" == cmd[0]
...
class TestStatefulSetCrateEnv:
    def test_without_ssl(self, faker):
        ...
        e_heap_size, e_java_opts = get_statefulset_crate_env(name, node_spec, None)
        assert e_heap_size.name == "CRATE_HEAP_SIZE"
        assert e_heap_size.value == "58812530"
        assert e_java_opts.name == "CRATE_JAVA_OPTS"
        assert e_java_opts.value_from.config_map_key_ref.key == "crate.java_opts"
        assert e_java_opts.value_from.config_map_key_ref.name == f"crate-{name}"
...
class TestStatefulSetCrateVolumeMounts:
    def test_without_ssl(self, faker):
        ...
        (
            ...
            vm_resource,
            *vm_data,
        ) = get_statefulset_crate_volume_mounts(node_spec, None)
        assert vm_log4j2.name == "crate-log4j2"
    ...
    def test_with_ssl(self, faker):
        ...
        (
            ...
            *vm_data,
            vm_ssl,
        ) = get_statefulset_crate_volume_mounts(node_spec, {})
        assert vm_log4j2.name == "crate-log4j2"
        assert vm_jmxdir.name == "jmxdir"
        assert vm_resource.name == "resource"
        assert [(vm.mount_path, vm.name) for vm in vm_data] == [
...
class TestStatefulSetVolumes:
    def test_without_ssl(self, faker):
        name = faker.domain_word()
        v_log4j2, v_sql_exporter, v_resource, v_jmx = get_statefulset_volumes(
            name, None
        )
        assert v_log4j2.name == "crate-log4j2"
    ...
    def test_with_ssl(self, faker):
        ...
        (
            ...
            v_jmx,
            v_keystore,
        ) = get_statefulset_volumes(name, ssl)
        assert v_log4j2.name == "crate-log4j2"
        assert v_sql_exporter.name == "crate-sql-exporter"
        assert v_resource.name == "resource"
        assert v_resource.persistent_volume_claim.claim_name == f"local-resource-{name}"
