...
async def check_reindexing_tables(
    ...
    if new_major > old_major:
        ...
        conn_coro = get_connection(host, password, timeout=CONNECT_TIMEOUT)
        async with await conn_coro as conn:
            cursor_coro = conn.cursor()
            async with await cursor_coro as cursor:
                query = f"""
                    SELECT table_name
                    FROM (
                        SELECT table_name,
