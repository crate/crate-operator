...
class FinalRollbackSubHandler(StateBasedSubHandler):
    ...
    async def handle(
        ...
        **kwargs: Any,
        ...
        logger.info(f"[{self.__class__.__name__}] Evaluating rollback conditions...")
    ...
    async def perform_rollback(self, **kwargs):
        ...
        logger.info(
            f"[{self.__class__.__name__}] Determining rollback class for "
        ...
        logger.info(
            f"[{self.__class__.__name__}] Starting rollback using handler: "
            f"{handler_name}"
        )
...
class RollbackHandler:
    ...
    def is_in_rollback(self) -> bool:
        val = self.body.metadata.annotations.get(self.annotation_key()) == "true"
        self.logger.info(f"[{self.annotation_key()}] is_in_rollback -> {val}")
        return val
...
class RollbackUpgradeSubHandler(RollbackHandler):
    def annotation_key(self):
        return f"{KOPF_STATE_STORE_PREFIX}/rollback-upgrade"
