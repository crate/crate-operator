...
- **Override/Configure from Labels**: Allows overriding configured settings, including decommission parameters
- **Set/Reset Routing Allocation**: Allows setting/resetting routing allocation for individual pods
  ...
  postStart:
    exec:
      command:
        ...
        - |
          ARCH=$(uname -m) &&
          curl -sLO {base_url}/dc_util-linux-${{ARCH}} &&
          curl -sLO {base_url}/dc_util-linux-${{ARCH}}.sha256 &&
    ...
    exec:
      comm<and:
        ...
        - |
          ...
          sha256sum -c dc_util-linux-${{ARCH}}.sha256 &&
          chmod +x dc_util-linux-${{ARCH}} &&
          ./dc_util-linux-${{ARCH}} --min-availability PRIMARIES || true
...
As already mentioned `terminationGracePeriodSeconds` MUST be set larger then `--timeout` otherwise kubelet will SIGKILL the container before decommission finished!
...
- **Disable postStart**: Set `dc-util-no-poststart: "true"`
