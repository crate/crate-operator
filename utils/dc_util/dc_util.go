...
// getNoPostStartFromLabels reads no-poststart setting from StatefulSet labels
func getNoPostStartFromLabels(labels map[string]string) bool {
	if value, exists := labels[labelNoPostStart]; exists {
		switch value {
		case "TRUE", "true", "True":
			log.Printf("Using no-poststart from StatefulSet label: true")
			return true
		case "FALSE", "false", "False":
			log.Printf("Using no-poststart from StatefulSet label: false")
			return false
		default:
			log.Printf("Invalid no-poststart value in StatefulSet label '%s': %s, using default: false",
				labelNoPostStart, value)
		}
	}
	return false // default behavior
}
...
func handleResetRouting(hostname string, dryRun bool, lockFile string) error {
	...
	if inClusterConfig {
		...
		if err != nil {
			log.Printf("Failed to create in-cluster config: %v - allowing pod startup", err)
			return nil
		}
	} else {
		...
		if err != nil {
			log.Printf("Failed to load kubeconfig: %v - allowing pod startup", err)
			return nil
		}
	}
	...
	if err != nil {
		log.Printf("Failed to create Kubernetes client: %v - allowing pod startup", err)
		return nil
	}
	...
	if err != nil {
		log.Printf("Failed to get namespace: %v - allowing pod startup", err)
		return nil
	}
	...
	if len(hostnameParts) < 2 {
		log.Printf("Invalid HOSTNAME format: %s - allowing pod startup", hostname)
		return nil
	}
	...
	if err != nil {
		log.Printf("Failed to get StatefulSet: %v - allowing pod startup", err)
		return nil
	}

	// 1. FIRST: Check if lock file exists
	if !lockFileExists(lockFile) {
		log.Println("No lock file found, normal startup, exiting")
		return nil
	}
	...
	if statefulSet.Spec.Replicas == nil || *statefulSet.Spec.Replicas <= 1 {
		...
		if replicaCount == 0 {
			log.Printf("StatefulSet has %d replicas, cluster is scaled down/suspended, removing lock file and exiting", replicaCount)
		} else {
			log.Printf("StatefulSet has %d replicas, single node cluster, removing lock file and exiting", replicaCount)
		}
	...
	if err := waitForClusterReadiness(proto, postStartTimeout, dryRun); err != nil {
		log.Printf("Cluster readiness check failed: %v - allowing pod startup", err)
	...
	if err := sendSQLStatement(proto, resetStmt, dryRun); err != nil {
		...
		log.Printf("This is expected during cluster formation - continuing to allow pod startup")
		// Remove lock file and continue (don't return error to avoid blocking pod startup)
		if removeErr := removeLockFile(dryRun, lockFile); removeErr != nil {
			log.Printf("Warning: Failed to remove lock file: %v", removeErr)
		}
		log.Printf("PostStart hook completing despite routing allocation failure to allow pod startup")
		return nil
	}
